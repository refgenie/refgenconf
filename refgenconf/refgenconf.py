...
__all__ = ["RefGenConf", "RefGenConf_old"]
...
class RefGenConf_old(yacman.YacAttMap):
    """ A sort of oracle of available reference genome assembly assets """
    ...
    def __init__(self, filepath=None, entries=None, writable=False, wait_max=60,
        ...
        super(RefGenConf_old, self).__init__(filepath=filepath, entries=entries,
                                             writable=writable, wait_max=wait_max,
                                             skip_read_lock=skip_read_lock)
        ...
        if not isinstance(genomes, PXAM):
            ...
            self[CFG_GENOMES_KEY] = PXAM()
        if CFG_FOLDER_KEY not in self:
            self[CFG_FOLDER_KEY] = os.path.dirname(
                entries) if isinstance(entries, str) else os.getcwd()
            _missing_key_msg(CFG_FOLDER_KEY, self[CFG_FOLDER_KEY])
        try:
            version = self[CFG_VERSION_KEY]
        except KeyError:
            ...
            try:
                ...
                if version < 0.3:
                    msg = "This genome config (v{}) is not compliant with v0.3 standards. To use it, please downgrade " \
                          "refgenie: 'pip install refgenie=={}'.".format(
                              self[CFG_VERSION_KEY], REFGENIE_BY_CFG[str(version)])
                    raise ConfigNotCompliantError(msg)
                else:
                    _LOGGER.debug(
                        "Config version is compliant: {}".format(version))
    ...
    def listr(self, genome=None, order=None, get_url=lambda server, id: construct_request_url(server, id), as_str=False):
        ...
        for url in self[CFG_SERVERS_KEY]:
            url = get_url(url, API_ID_ASSETS)
            data_by_server[url] = _list_remote(
                url, genome, order, as_str=as_str)
        return data_by_server
    ...
    def tag(self, genome, asset, tag, new_tag, files=True):
        ...
        """
        self.run_plugins(PRE_TAG_HOOK)
        ori_path = self.seek(genome, asset, tag,
                             enclosing_dir=True, strict_exists=True)
        new_path = os.path.abspath(os.path.join(ori_path, os.pardir, new_tag))
        if self.file_path:
            with self as r:
                if not r.cfg_tag_asset(genome, asset, tag, new_tag):
    ...
    def _update_genome_servers(self, url, reset=False):
        ...
        if CFG_SERVERS_KEY in self:
            ...
            raise GenomeConfigFormatError(
                "The '{}' is missing. Can't update the server list".format(CFG_SERVERS_KEY))
    ...
    def write(self, filepath=None):
        ...
        """
        self.run_plugins(PRE_UPDATE_HOOK)
        path = super(RefGenConf_old, self).write(filepath=filepath)
        self.run_plugins(POST_UPDATE_HOOK)
        return path
    ...
    def config_upgrade(self, target_version, force=False):
        ...
        def alter_file_tree(config):
            ...
            os.mkdir(config[CFG_FOLDER_KEY]+'/alias/')
            # move folder
            for root, dirs, files in os.walk(config[CFG_FOLDER_KEY]):
                for dir in dirs:
                    if dir in my_genome:
                        shutil.move(config[CFG_FOLDER_KEY]+"/" + dir,
                                    config[CFG_FOLDER_KEY]+'/data/' + dir)
                del dirs[:]

            for root, dirs, files in os.walk(config[CFG_FOLDER_KEY]+'/data/'):
                for dir in dirs:
                    _swap_names_in_tree(root+dir, my_genome[dir], dir)
                    os.mkdir(config[CFG_FOLDER_KEY]+'/alias/'+dir)
