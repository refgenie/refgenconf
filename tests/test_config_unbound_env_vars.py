...
def test_missing_env_vars_in_genome_config_path_raises_exception(rgc, tmpdir, evs, genome, asset, tag, gencfg,
    ...
    with pytest.raises(UEVErr) as err_ctx:
        rgc.pull_asset(genome=genome, asset=asset, tag=tag, genome_config=cfg_file_copy, get_json_url=get_get_url(genome, asset))
